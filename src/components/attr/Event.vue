  ...
  <el-card class="eventEditor">
    <el-collapse v-model="activeNames" @change="handleChange">
      <el-collapse-item
        :title="item.event"
        :name="index"
        v-for="(item, index) in inContent"
        :key="item.event"
      >
        <div slot="title">
          {{ item.event }}
          <i
            @click="delectFunc(item.event)"
            class="header-icon el-icon-delete"
          ></i>
        </div>
        <attr-function
          :targetmethods="[]"
          :content.sync="item.callList"
        ></attr-function>
      </el-collapse-item>
    </el-collapse>
    <div class="addScriptWarp">
      <el-select
        class="scriptselect"
        size="mini"
        v-model="selectEventKey"
        placeholder="请选择"
      >
        <el-option
          v-for="item in eventList"
          :key="item.value"
          :label="item.value + '->' + (item.label || item.desc)"
          :value="item.value"
        >
        </el-option>
      </el-select>
      <el-button type="primary" size="mini" @click="addEvent" :disabled="canAdd"
        >添加事件</el-button
      >
    </div>
  </el-card>
