<template>
  <div class="functionEditor">
    <el-form label-width="80px">
      <el-form-item label="目标对象">
        <el-cascader
          ...
          :props="{ value: 'id', children: 'child' }"
          :options="options"
          v-model="selectedOptions"
          @change="handleChange"
          @focus="loadNodeAndFunction"
          change-on-select
        >
        </el-cascader>
      </el-form-item>
      <div v-if="targetmethods.length > 0">
        ...
        <div>
          <el-tag
            @click.native="methodSelect(item)"
            size="small"
            class="tags"
            :type="getTagType(item.name)"
            v-for="(item, key) in targetmethods"
            :key="item.name"
          >
            {{ item.label }}
          </el-tag>
    ...
    <el-collapse
      ...
      class="collapse"
      accordion
      v-show="methodNames && methodNames.length"
    >
      <el-collapse-item
        class="panel"
        :name="methodName"
      ...
      >
        <template slot="title">
          <div
            v-for="exist in [fnExist(methodName)]"
            :key="methodName + exist"
            :style="{ color: !exist ? 'red' : 'inherit' }"
          >
            <i class="iconfont icon-function"></i>
            {{ !!exist ? methodName : "不存在该方法 " + methodName }}
            <i
              @click="delectFunc(methodName)"
        ...
        >
          ...
          >
            <el-form-item
              v-for="(item, key) in params(methodName)"
              :key="key"
              class="form-item"
            >
              <span
              ...
              <el-input
                ...
                size="mini"
                @change="paramsChange"
                v-model.lazy="item.default"
              ></el-input>
              <el-input
                v-else-if="item.type == 'number'"
                size="mini"
                @change="paramsChange"
                v-model.lazy.number="item.default"
              ></el-input>
              <el-switch
              ...
              <template v-else-if="item.type == 'array'">
                <attr-data
                  v-if="isOpen(methodName)"
                  type="array"
                  @change="paramsChange"
                  :content.sync="item.default"
                ></attr-data>
              </template>
  ...
  methods: {
    ...
    getTargetMethod (target) {
      ...
      if (target) {
        ...
        // 获取当前元素的所有可调用方法列表
        var editorMethods = Object.assign(
          {},
          targetVm.$options.editorMethods,
          targetVm.$options.editerMethods
        );
      ...
      if (this.allTargetmethods[target.nodeInfo.id]) {
        this.dealTargetMethodsChange(
          targetmethods,
          this.allTargetmethods[target.nodeInfo.id]
        );
      } else {
