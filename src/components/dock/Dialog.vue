<template>
  <div
    class="ui-dock-dialog comp-panel-frame animation-panel"
    :style="info.style"
    :class="{moving: active, 'for-main-dock': isForMainDock, [dockedPos]: true}"
    @mousedown.capture="captureMousedown"
  >
    <header
      class="panel-header dragger dark"
    ...
    <div
      :class="[ labelRight ? 'label-right' : 'label-left' ]"
      class="tools"
      @mousedown="mousedown($event)"
    >
      <div class="tools-inner">
        <div class="btnWarp"><i
            ...
            class="el-icon-close btn"
            @mousedown.stop
            @click.stop="close"
          ></i></div>
        <div
          class="btnWarp"
          v-if="dockedPos === 'right-stick' || dockedPos === 'right-normal' || dockedPos === 'left-end'"
        ><i
            class="iconfont icon-double-right-filled btn"
            @mousedown.stop
            @click.stop="autoFold()"
          ></i></div>
        <div
          class="btnWarp"
          v-if="dockedPos === 'left-stick' || dockedPos === 'left-normal' || dockedPos === 'right-end'"
        ><i
            class="iconfont icon-double-left-filled btn"
            @mousedown.stop
            @click.stop="autoFold()"
          ></i></div>
        <div
          class="panel-names"
          v-show="dockedPos.includes('end')"
        >
          <div
            :key="i"
            class="panel-names-item"
            v-for="(item, i) in pannelNames"
          >{{item}}</div>
        </div>
      </div>
    </div>
...
.ui-dock-dialog
  ...
  &.moving
    opacity 0.95
    z-index 15
  &.left-normal:hover, &.right-normal:hover
    z-index 15
    transition z-index 100ms ease-in-out 100ms
  &.for-main-dock
    &:before
