  ...
  >
    ...
    >
      <a
        ...
        @click="revocation"
      >
        <i class="iconfont icon-revocation" />
      </a>
    </el-tooltip>
    <el-tooltip
      class="item"
    ...
    >
      <a
        ...
        @click.stop="psd"
      >
        <i class="iconfont el-icon-picture-outline" />
      </a>
    </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    preview () {
      ...
      this.ema.fire('pageInfo.save', true, () => {
        if (/desktop/i.test(this.Setting.phoneSize.name)) {
          return window.open(
            `${this.Config.previewHost}` + urlInfo.params.key + '?preview=1'
          );
        }
        this.openDialog({
          name: 'd-Preview',
    ...
    computeRight () {
      ...
      if (!rect) return;
      this.right =
        (rect.width - parseFloat(this.Setting.phoneSize.width)) / 2 - 80 + 'px';
    },
  },
};
</script>
