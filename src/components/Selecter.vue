  ...
  methods: {
    ...
    updateLine (ev) {
      ...
      function judge (rect, current, array, center = true) {
        ...
        if (
          center &&
          Math.abs(current.left - (rect.left + rect.width / 2)) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(current.left + current.width / 2 - rect.left) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(
            current.left + current.width / 2 - (rect.left + rect.width / 2)
          ) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(current.left + current.width / 2 - rect.right) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(current.right - (rect.left + rect.width / 2)) < baseOffset
        ) {
          array.x1.push(parseInt(rect.left + rect.width / 2));
        }
        ...
        if (
          center &&
          Math.abs(current.top - (rect.top + rect.height / 2)) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(current.top + current.height / 2 - rect.top) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(
            current.top + current.height / 2 - (rect.top + rect.height / 2)
          ) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(current.top + current.height / 2 - rect.bottom) < baseOffset
        ) {
        ...
        if (
          center &&
          Math.abs(current.bottom - (rect.top + rect.height / 2)) < baseOffset
        ) {
    ...
    getSorbPos (ev) {
      ...
      var left = !this.startPos.rightControl
        ? this.startPos.left
        : this.startPos.parentSize.width -
        this.startPos.right -
        this.startPos.w;
      var pos = {
    ...
    dealMove (ev) {
      ...
      switch (type) {
        ...
        case 'm':
          ...
          if (bottomControl) {
            bottom =
              this.startPos.parentSize.height - this.startPos.h - pos.top;
          } else {
            top = pos.top;
          }
          break;
    ...
    mousedown (ev, type) {
      ...
      this.allRectProps.map((dis) => {
        let val = size[dis];
        if (dis == 'left' && size.rightControl) { this.$set(thisNode.nodeInfo.style, 'left', null); } else if (dis == 'top' && size.bottomControl) { this.$set(thisNode.nodeInfo.style, 'top', null); } else if (!isUnset(val) && !/%$/.test(thisNode.nodeInfo.style[dis])) { this.$set(thisNode.nodeInfo.style, dis, val + 'px'); }
      });
    ...
    moveLayer (number) {
      ...
      if (!$parent) return;
      typeof $parent.moveLayer === 'function' &&
        $parent.moveLayer($node.nodeInfo, number);
    },
    getColumns () {
