<template>
  <div class="code-panel ns65536">
      ...
      >
        ...
        >
          <span
            ...
            slot="label"
            :title="item.comId + '_' + item.name + '_' + item.id"
          >{{item.name}} <i v-show='item.dirty'>*</i></span>
          <code-editor
            ctype='javascript'
            :highlight='item.highlight'
            :dirty.sync="item.dirty"
            :ctrls='true'
            @ctrls='item.onSave'
            :options="{}"
        ...
        >
          <span
            ...
            slot="label"
          >
            <i :class="{'el-icon-search': !searching, 'el-icon-loading': searching}" />
            <i class="" /> 搜索
          </span>
          <div class="search-section">
            ...
            >
              <el-checkbox-group
                slot="append"
                v-model="pickedSearchRules"
                size="mini"
              >
                <el-checkbox-button
                  v-for="(rule) in searchRules"
                  :label="rule.type"
                  :key="rule.type"
                >{{rule.name}}</el-checkbox-button>
              </el-checkbox-group>
            </el-input>
            <div class="search-result">
              <div
                v-show="searchResult === '__NULL__'"
                style="font-size: 12px;margin: 7px 3px;"
              >找不到结果</div>
              <template v-if="searchResult && searchResult.map">
                ...
                >
                  ...
                  <div
                    @click="openFile(item.file, {comId: item.comId, highlight: item.highlight})"
    ...
    <div
      v-else
      class="codepanel-hint"
      style="padding-top: 1px; pointer-events:none;"
    >
      <div class="codepanel-logo">TIANGG</div>
      <div class="hint-item">
