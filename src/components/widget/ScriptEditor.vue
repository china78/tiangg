    ...
    <div class="components">
      ...
      >
        <el-tag
          ...
          type="warning"
          class="addScriptWarp"
          v-for="item in scriptArray"
          :key="item.categoryId + '_' + item.id"
        >
          <div
            size="mini"
            @click="addScript(item)"
          >
        ...
        >
          ...
          <div class="iconwarp">
            <i
              ...
              :class="{'runtime': value.runtimeOnly}"
              :title="value.runtimeOnly ? '仅运行时执行' : '始终执行'"
              @click.stop="toogleRuntimeOnly(value)"
            />
            <i
              class="iconfont icon-download"
              :style="{transform: 'rotate(180deg)'}"
              title="保存到脚本库"
              @click.stop="saveTemplate(value)"
            />
            <i
              class="iconfont icon-rename"
              title="重命名"
              @click.stop="renameItem(value)"
            />
            <i
