<template>
  ...
  >
    ...
    <div @click="opend = !opend" class="show-more">
      {{ !opend ? "查看更多标签▼" : "收起▲" }}
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    onclick(tag) {
      ...
      this.$emit(
        "selectchange",
        this.sortedTags.filter((v) => v.selected)
      );
    ...
    initHistoryTags() {
      var tags = (window.localStorage.getItem(this.ASSETS_TAG_KEY) || "")
        .split(",")
        .filter((v) => v && v.length);
